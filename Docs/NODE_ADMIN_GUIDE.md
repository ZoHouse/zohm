...
This document outlines the database schema, types, and requirements for building/updating the Node Management Admin page to support the new city coordination layer with **19 node types** and **13 zone types**.
...
| `type` | TEXT | One of 19 node types (see below) |
...
| `staynode` | 🏨 (Logo) | Zostel | Legacy alias for `zostel` |
...
type NodeType =
  | 'zo_house'
  | 'zostel'
  | 'staynode'    // Legacy alias for zostel
  | 'food' 
  | 'stay' 
  | 'park' 
  | 'hospital' 
